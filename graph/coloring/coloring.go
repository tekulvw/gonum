...
func WelshPowell(g graph.Undirected, partial map[int64]int) (k int, colors map[int64]int, err error) {
	...
	k, colors = greedyColoringOf(g, newSaturationDegreeIterator(nodes, g), partial)
	return k, colors, nil
}
...
// newSaturationDegreeIterator returns a graph.Node iterator that returns nodes
// in order of descending degree.
func newSaturationDegreeIterator(it graph.Nodes, g graph.Undirected) graph.Nodes {
	nodes := graph.NodesOf(it)
